...
public final class QtiDcTracker extends DcTracker {
    ...
    public QtiDcTracker(Phone phone) {
        ...
        if (phone.getPhoneType() == PhoneConstants.PHONE_TYPE_CDMA) {
            ...
            boolean featureOverride = SystemProperties.getBoolean(OMH_FEATURE_ENABLE_OVERRIDE, false);
            if (featureOverride) {
            ...
            if (fetchApnFromOmhCard) {
                ...
                this.mOmhApt.registerForModemProfileReady(this, EVENT_MODEM_DATA_PROFILE_READY, null);
            }
        }
        fillIccIdSet();
    }
    ...
    private boolean getConfigItem(String key) {
        CarrierConfigManager carrierConfigManager = (CarrierConfigManager) this.mPhone.getContext()
                                                                .getSystemService("carrier_config");
    ...
    private void onModemApnProfileReady() {
        ...
        cleanUpConnectionsOnUpdatedApns(tearDown);
    ...
    protected void onRecordsLoadedOrSubIdChanged() {
        ...
        this.mAutoAttachOnCreationConfig = this.mPhone.getContext().getResources()
    ...
    protected void createAllApnList() {
        ...
        String operator = "";
        IccRecords iccRecords = (IccRecords) this.mIccRecords.get();
        operator = this.mPhone.getPhoneType() == PhoneConstants.PHONE_TYPE_GSM
                       ? iccRecords != null ? iccRecords.getOperatorNumeric()
                       : "" : isNvSubscription()
                       ? SystemProperties.get("ro.cdma.home.operator.numeric")
                       : iccRecords != null ? iccRecords.getOperatorNumeric() : "";
        int radioTech = this.mPhone.getServiceState().getRilDataRadioTechnology();
        if (!(this.mOmhApt == null || !ServiceState.isCdma(radioTech)
        ...
        if (!(!this.mAllApnSettings.isEmpty() || operator == null || operator.isEmpty())) {
            ...
            Cursor cursor = this.mPhone.getContext().getContentResolver()
                      .query(Carriers.CONTENT_URI, null, selection, null, "_id");
            if (cursor != null) {
    ...
    protected boolean allowInitialAttachForOperator() {
        IccRecords iccRecords = (IccRecords) this.mIccRecords.get();
        String iccId = iccRecords != null ? iccRecords.getIccId() : "";
        if (iccId != null) {
    ...
    protected void log(String string) {
        Rlog.d(this.LOG_TAG, "[" + this.mPhone.getPhoneId() + "]" + string);
    }
}
