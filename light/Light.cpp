...
Light::Light(std::pair<std::ofstream, uint32_t>&& lcd_backlight,
             std::pair<std::ofstream, uint32_t>&& button_backlight,
             std::ofstream&& red_led,
             std::ofstream&& green_led,
             std::ofstream&& blue_led,
             std::ofstream&& red_breath,
             std::ofstream&& green_breath,
             std::ofstream&& blue_breath,
...
void Light::setLcdBacklight(const LightState& state) {
    ...
    // If max panel brightness is not the lcd default (255),
...
void Light::setButtonsBacklight(const LightState& state) {
    ...
    // Mi Max buttons brightness range is 0-40,
    // apply linear scaling across that range.
    brightness /= DEFAULT_MAX_BRIGHTNESS / mButtonBacklight.second;
...
void Light::setSpeakerLightLocked(const LightState& state) {
    ...
    // Do everything with the lights out...
    mRedBreath << breath_pattern << std::endl;
    mRedBlink << (blink && red ? 1 : 0) << std::endl;
    mGreenBreath << breath_pattern << std::endl;
    mGreenBlink << (blink && green ? 1 : 0) << std::endl;
    mBlueBreath << breath_pattern << std::endl;
